...
from llamaduck.cli import chat, cli, search
...
class TestCLI(unittest.TestCase):
    ...
    def test_search_command_with_options(self, mock_ddgs):
        ...
        result = self.runner.invoke(
            search, ["python test", "--limit", "3", "--region", "us-en"]
        )

        # Assertions
        self.assertEqual(result.exit_code, 0)
        mock_instance.text.assert_called_once_with(
            "python test", region="us-en", max_results=3
        )
    ...
    def test_chat_command(self, mock_ddgs):
        ...
        mock_instance = MagicMock()
        mock_ddgs.return_value.__enter__.return_value = mock_instance
        mock_instance.text.return_value = [{"body": "Python is a programming language"}]
